...
/* 🔴🔴 fsd_burst_fill_last_act() IS GONE (2026-09-10). Its only consumer was
 * FsdBodyInputs.last_act_ms, which fed min_interval_ms -- and the owner removed
 * every safety gate, so both went. Keeping the function would have left one
 * nothing can call, which is this repository's oldest failure shape. */
