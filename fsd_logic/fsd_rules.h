/* 🔴 화면에서는 이것을 **"매핑"** 이라고 부른다 (차주 지시 2026-09-03).
 *
 * 코드 이름은 `Rule` 그대로다 — 앱의 `Rules.kt`, BLE 의 `RULE_SET`/`RULE_CLEAR`,
 * 이 파일, 그리고 시리얼 태그 `[RULES]` 가 전부 그 이름을 쓴다. 한쪽만 바꾸면
 * **대조가 끊긴다**: 이 저장소가 `OpMode`(`Active`/`Listen-Only`)에서 같은
 * 판단을 한 적이 있고, 그때도 이름은 두고 화면에만 풀이를 붙였다.
 *
 * 사람에게 보이는 한국어(앱 문구·시리얼 문장)는 "매핑" 이고, 식별자는 `Rule` 이다.
 * 키토스 앱이 같은 것을 "매핑" 탭이라 부르고 차주에게 익은 말이 그쪽이다.
 */
