...
bool fsd_burst_arm(FsdBurst* b, FsdBodyAction action, int32_t arg, uint8_t rule_index,
    ...
    for (unsigned i = 0; i < FSD_BURST_MAX; i++) {
        ...
        s->seq = b->next_seq++;
        /* 🔴 THE COMMAND USED TO BE STAMPED HERE, for min_interval_ms. That
         * gate went with the rest of the axis on 2026-09-10 (owner's
         * instruction), and the stamp went with it rather than being left to
         * accumulate for a reader that no longer exists. */
        return true;
    }
