...
/* ── the phone's fix on the wire ──────────────────────────────────────────────
 *
 * 🔴 THIS LIVES HERE RATHER THAN IN ble_server.cpp ON PURPOSE. That file is not
 * in the host build, so a byte layout written there is a layout no test can
 * reach — this repo has been bitten by exactly that shape more than once (the
 * OTA verdict and the battery field each existed twice, and only one copy was
 * ever tested). Unpacking here means the app's encoder and the module's decoder
 * are bound by the shared vectors in test/fixtures/wire_vectors.json.
 *
 * 17 bytes, little-endian, after the command byte:
 *
 ...
 *   [12..13] uint16 speed in 0.01 km/h           — recorded, NOT used
 *   [14..16] uint24 how old the fix already was when the phone sent it
 */
#define FSD_GPS_BLE_FIX_LEN 17u

typedef struct {
    int32_t lat_e7;
    int32_t lon_e7;
    float accuracy_m;  // 0 means unknown, the FsdCamFix convention
    float bearing_deg;
    /* 🔴 CARRIED, NOT BELIEVED. fsd_gps_fix_why() fills FsdCamFix.speed_kph from
     * 0x257 — the drivetrain — because a phone in a tunnel keeps reporting the
     * last speed it saw. This value exists so a bring-up log can put the two
     * side by side, and for nothing else. */
    float speed_kph;
    uint32_t age_ms;
} FsdGpsBleFix;

/** Unpack the BLE_CMD_GPS_FIX payload. Returns false — leaving *out untouched —
 *  when the payload is short. Range checking is NOT done here: that belongs to
 *  fsd_gps_observe_phone(), which applies the same bounds the frame decoder
 *  does, so there is one place that decides what is a place on Earth. */
